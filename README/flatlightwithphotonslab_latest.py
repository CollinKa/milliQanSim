...
# libBenchCore/libMilliQanCore must be read with the same ROOT version used to build
# the simulation (milliqan-sim on macOS). Using root_env or system python3 causes
# "no dictionary for class mqROOTEvent" and missing GetEventID().
def load_milliqan_dictionary(repo_dir):
    build_dir = repo_dir / "build"
    include_dir = repo_dir / "include" / "Bench"
    if include_dir.exists():
        ROOT.gInterpreter.AddIncludePath(str(include_dir))

    # Let ROOT discover libBenchDict.rootmap / libBenchDict_rdict.pcm in build/
    ROOT.gSystem.AddDynamicPath(str(build_dir))

    library_candidates = [
        build_dir / "libBenchCore.dylib",
        build_dir / "libMilliQanCore.dylib",
        build_dir / "libBenchCore.so",
        build_dir / "libMilliQanCore.so",
    ]

    for library_path in library_candidates:
        if not library_path.exists():
            continue
        load_status = ROOT.gSystem.Load(str(library_path))
        if load_status < 0:
            continue
        try:
            test_event = ROOT.mqROOTEvent()
        except AttributeError:
            continue
        if hasattr(test_event, "GetEventID"):
            return library_path

    tried = "\n  ".join(str(path) for path in library_candidates)
    raise RuntimeError(
        "Failed to load MilliQan ROOT dictionary (mqROOTEvent::GetEventID missing).\n"
        "Use the same conda env as the Geant4 build, then run with that env's python:\n"
        "  conda activate milliqan-sim\n"
        "  python flatlightwithphotonslab.py\n"
        f"Current ROOT: {ROOT.gROOT.GetVersion()}\n"
        "Tried libraries:\n  " + tried
    )
